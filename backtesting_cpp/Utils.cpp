...
vector<double>,vector<double> > rearrange_candles(double** candles, string tf, long long from_time, long long to_time, int& array_size) {
    ...
    for (int i = 0; i < array_size ; i++) {
        ...
        if (candles[i][0] < from_time) continue;
        if (candles[i][0] >= current_ts + tf_ms) {
            ...
            if (candles[i][2] > current_h) current_h = candles[i][2];
            if (candles[i][3] < current_l) current_l = candles[i][3];
            current_c  = candles[i][4];
            current_v += candles[i][5];
        }
    }
